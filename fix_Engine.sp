...
//Царапка: если танк подожженный зажигательными патронами зайдет в обычный огонь (от молотова или от канистры), то он все равно потушится.
...
static		Handle:g_Warning, Handle:g_HRate, Handle:Timer[MAXPLAYERS+1], count[MAXPLAYERS+1], chat[MAXPLAYERS+1],
			Float:g_CvarHRate, bool:g_CvarWarn, bool:bEvent;
...
public OnMapStart()
{
	if (IsNoMercyMap() && !bEvent){
		LogMessage("Hook EVENT");
		HookEvent("round_start", Event_RoundStart, EventHookMode_PostNoCopy);
		bEvent = true;
	}
	else if (!IsNoMercyMap() && bEvent){
		LogMessage("UnHook EVENT");
		HookEvent("round_start", Event_RoundStart, EventHookMode_PostNoCopy);
		bEvent = false;
	}
}
...
{
	PrintToChatAll("RR");
	CreateTimer(12.4, NoMercyFix, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}

public Action:NoMercyFix(Handle:timer)
{
	decl Float:fEyePos[3];
	
	for (new i = 1; i <= MaxClients; i++){
		if (IsClientInGame(i) && GetClientTeam(i) == 2){
			GetClientEyeAngles(i, fEyePos);
			fEyePos[1] = 179.0;
			PrintToChatAll("%N", i);
			TeleportEntity(i, NULL_VECTOR, fEyePos, NULL_VECTOR);
		}
	}
}
