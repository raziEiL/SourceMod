...
new		Handle:g_hCvarBarrowFix, Handle:g_hCvarTimer, bool:g_bCvarBarrowFix, Float:g_fCvarTimer, bool:g_bAllowHook;
...
public OnPluginStart()
{
	...
	g_hCvarBarrowFix	=	CreateConVar("physics_barrow", "0", "WheelBarrow Fix: 1: Enable, 0: Disable", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	g_hCvarTimer			=	CreateConVar("physics_timer", "0.01", "Time interval when the fix is triggered after oxygentank/fireworkcrate/propanetank created, 0.0: Disable Physics Fix", FCVAR_PLUGIN, true, 0.0, true, 2.0);
...
public OnEntityCreated(entity, const String:classname[])
{
	if (entity != -1 && IsValidEntity(entity)){
		...
		else if (g_bAllowHook && g_fCvarTimer && 
				(strcmp(classname, OXYGENTANK) == 0 ||
				strcmp(classname, FIREWORKCRATE) == 0 ||
				strcmp(classname, PROPANETANK) == 0)){
...
PhysicsFixFunc(entity, String:sModel[], bool:bOverride = false)
{
