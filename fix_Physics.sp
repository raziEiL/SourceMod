...
/*=====================
	   < ConVar >
=======================*/
static	Handle:g_Physics, Handle:g_Timer, Handle:g_Exception, g_CvarPhysics, w, 

		Float:g_CvarTimer, String:Models[64], bool:hook, bool:disable;
...
public OnPluginStart()
{
	...
	g_Timer		=	CreateConVar("physics_timer", "0.01", "When created entnity change class after x.x sec; 0.0 Disable Physics", FCVAR_PLUGIN, true, 0.0, true, 2.0);
	g_Exception	=	CreateConVar("physics_exception", "models/props/cs_office/Fire_Extinguisher.mdl", "l4d_extinguisher", FCVAR_PLUGIN);
...
{
	if (entity != -1 && IsValidEdict(entity) && IsValidEntity(entity) && !IsException(entity)){
...
bool:IsException(entity)
{
	decl String:vModels[64], String:g_CvarExt[1024];
	
	GetEntPropString(entity, Prop_Data, "m_ModelName", vModels, sizeof(vModels));
	GetConVarString(g_Exception, g_CvarExt, sizeof(g_CvarExt));
	
	if (StrContains(g_CvarExt, vModels) != -1){
		#if debug
		PrintToChatAll("Exception <%s>", vModels);
		LogMessage("Exception entity!");
		#endif
		return true;
	}
	return false;
...
GetCVars()
{
	...
	if (g_CvarTimer == 0) disable=true;
	else disable=false;
	
	#if debug
	PrintToChatAll("disabe %d", disable);
	LogMessage("disabe %d", disable);
	#endif
}
