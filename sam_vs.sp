...
static	Float:g_fCvarKickT, g_iCvarAdmFlag, bool:g_bCvarImBack, g_iCvarSpecT, bool:g_bCvarTank, Handle:g_hTimer[MAXPLAYERS+1],
		Float:g_fButtonTime[MAXPLAYERS+1], bool:g_bTempBlock[MAXPLAYERS+1], bool:g_bLeft4Dead2, bool:g_bLoadLate;
...
public OnPluginStart()
{
	...
	new Handle:hSpecT		= CreateConVar("sam_vs_spec_time",		"35", "Time before idle player will be moved to spectator in seconds", FCVAR_PLUGIN, true, 10.0);
	new Handle:hKickT		= CreateConVar("sam_vs_kick_time",		"120", "Time before idle spectator player will be kicked in seconds. 0 = never kick", FCVAR_PLUGIN, true, 0.0);
...
public Action:SAM_ev_PlayerSay(Handle:event, String:event_name[], bool:dontBroadcast)
{
	...
	if (GetClientTeam(client) != 1){
		...
		#if debug
			PrintToChatAll("[IDLE debug] %N chatting", client);
		#endif
...
{
	#if debug
		PrintToChatAll("[IDLE debug] %N pressed a button (%d)", client, GetClientButtons(client));
	#endif
...
{
	static Float:fTheTime, bool:bRUP, iTeam;
	fTheTime = GetEngineTime(), bRUP = IsReadyUpActive();

	#if debug
		PrintToChatAll("[IDLE debug] SAM_t_CheckIdles()-> fired, RUP = %b!", bRUP);
	#endif
		...
		if (g_fButtonTime[i] && (fTheTime - g_fButtonTime[i]) > g_iCvarSpecT){
			...
			if (IsClientInGame(i) && (iTeam = GetClientTeam(i)) != 1){
				...
				if (IsPlayerBussy(i, iTeam) || g_bCvarTank && iTeam == 3 && IsPlayerTank(i)){
					...
					#if debug
						PrintToChatAll("[IDLE debug] SAM_t_CheckIdles()-> %N are bussy/tank. We skip him..", i);
					#endif
				...
				#if debug
					PrintToChatAll("[IDLE debug] SAM_t_CheckIdles()-> %N moved to spec (Was AFK %.1fsec)", i, fTheTime - g_fButtonTime[i]);
				#endif

				if (bWhoAmI(i) || !g_fCvarKickT) return;

				g_hTimer[i] = CreateTimer(g_fCvarKickT, SAM_t_ActionKick, i, TIMER_FLAG_NO_MAPCHANGE);

				#if debug
					PrintToChatAll("[IDLE debug] SAM_t_CheckIdles()-> %N AFK, timer %x hndl", i, g_hTimer[i]);
				#endif
			}
			else
				g_fButtonTime[i] = 0.0;
